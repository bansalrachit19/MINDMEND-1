...
export default function MoodSummaryWidget({ user }) {
  const { setShowMoodModal } = useMood();
  const mood = user?.moodCategory || 'Unknown';
  const last = user?.lastMoodCheck ? new Date(user.lastMoodCheck).toLocaleDateString() : 'N/A';
  ...
  return (
    <div className="bg-white shadow-xl rounded-2xl p-6 w-full max-w-md">
      ...
      <p className="text-gray-700 mb-1">Current Mood Category: <strong>{mood.toUpperCase()}</strong></p>
      <p className="text-gray-700 mb-2">Last Checked: {last}</p>
      <p className="text-sm italic text-gray-600">{moodTips[mood]}</p>
      <button
